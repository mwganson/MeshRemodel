...
"Sit tight" = do nothing, "Haul vessel" = do full transformation to destination, "Translate only" = Haul vessel to destination, but do not rotate it, and "Rotate only" = Leave the vessel where it is, but rotate into alignment with the destination relative to how the tugboat is aligned to the vessel.  After every command is carried out the property resets itself to "Sit tight".
...
#### Release notes:
* 2026.09.05 (1.12.0) -- for ParametricMesh objects set Relative to True by default as this can prevent some cases of self-intersections in the mesh
* 2026.03.15 (1.11.0) -- updated for newer 1.2.0 and later FreeCAD versions, changed folder hierarchy mostly.
...
* 2024.09.22 (1.10.24) -- fix addon manager check for updates after addon manager code change
...
* Support Draft points in addition to Part::Vertex objects for grid insertion (part::feature python objects with X,Y,Z properties and only 1 vertex in shape)
